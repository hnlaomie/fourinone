	...
	public ArrayList getXmlObjectByFile(String filePath, String PROPSROW_DESC, String KEY_DESC)
	{
		...
		*/
		...
		try
		{
			...
			SAXParserFactory factory = SAXParserFactory.newInstance(); 
			SAXParser saxParser = factory.newSAXParser();
			//InputSource src = new InputSource(nem(filePath));
			InputSource src = new InputSource(filePath);
			if(PROPSROW_DESC!=null)
				handler.setPROPSROW_DESC(PROPSROW_DESC);
			if(KEY_DESC!=null)
				handler.setKEY_DESC(KEY_DESC);
			...
			System.err.println("[XmlConfig][Can't find config file:"+filePath+",and create default config.xml in the pat配置文件不存在则从运行环境中读取h.]");
